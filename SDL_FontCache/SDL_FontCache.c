...
FC_Rect FC_VA_Draw(FC_Font* font, FC_Target* dest, float x, float y, const char* text)
{
    if(text == NULL || font == NULL)
        return FC_MakeRect(x, y, 0, 0);

    strcpy(fc_buffer, text);
...
/*
* Load and create a new font at the given FC_Font* pointer.
* Checks for and logs errors using SDL_Log.
* @return 0 on success, -1 for error.
*/
int FC_NewFont(FC_Font ** font, SDL_Renderer *renderer, const char *filename_ttf, Uint32 pointSize, SDL_Color color, int style) {
    if (!font) {
        SDL_Log("Error: font pointer parameter making new font is null.");
        return -1;
    }
    if (!filename_ttf) {
        SDL_Log("Error: Filename is NULL when creating new font.");
        return -1;
    }

    // This shouldn't cause any errors, but it's just to enforce good practice,
    // and make sure fonts are default initialized NULL.
    if ((*font) != NULL) {
        SDL_Log("Warning: FC_Font*'s value was not NULL when creating new font.");
    }

    *font = FC_CreateFont();
    // FontCache returns 1 on success, 0 on error for some reason...
    if (FC_LoadFont(*font, renderer, filename_ttf, pointSize, color, style) == 0) {
        // Failed to load font, but the function already logs the error,
        // so logging something here would be kinda pointless.
        return -1;
    }
...
int FC_GetSize(FC_Font* font) {
    return font->height;
}

// Possible TODO: add vertical alignment functions for scale and other options.

FC_Rect FC_DrawAlignV(FC_Font* font, FC_Target* dest, float x, float y, FC_AlignEnum alignHorizontal, FC_VerticalAlignEnum alignVertical, const char* formatted_text, ...)
{
    if(formatted_text == NULL || font == NULL)
        return FC_MakeRect(x, y, 0, 0);
    ...
    {
        case FC_V_ALIGN_TOP:
            // top alignment is the default alignment, nothing to do.
            break;
        case FC_V_ALIGN_CENTER:
            y = y - font->height/2; 
            break;
        case FC_V_ALIGN_BOTTOM:
            // I hope the font size correlates 1 to 1 with target texture height, it may not though.
            y = y - font->height;
        default:
            break;
    }
