...
#define MAP_LIST_OR0(f, x, peek, ...) | f(x) DEFER ( MAP_NEXT(peek, MAP_LIST_OR1) ) ( f, peek, __VA_ARGS__ ) 
#define MAP_LIST_OR1(f, x, peek, ...) | f(x) DEFER ( MAP_NEXT(peek, MAP_LIST_OR0) ) ( f, peek, __VA_ARGS__ ) 
#define MAP_LIST_OR2(f, x, peek, ...)   f(x) DEFER ( MAP_NEXT(peek, MAP_LIST_OR1) ) ( f, peek, __VA_ARGS__ ) 

#define MAP_LIST0_UD_OR(f, userdata, x, peek, ...) | f(x, userdata) DEFER ( MAP_NEXT(peek, MAP_LIST1_UD_OR) ) ( f, userdata, peek, __VA_ARGS__ ) 
#define MAP_LIST1_UD_OR(f, userdata, x, peek, ...) | f(x, userdata) DEFER ( MAP_NEXT(peek, MAP_LIST0_UD_OR) ) ( f, userdata, peek, __VA_ARGS__ ) 
#define MAP_LIST2_UD_OR(f, userdata, x, peek, ...)   f(x, userdata) DEFER ( MAP_NEXT(peek, MAP_LIST1_UD_OR) ) ( f, userdata, peek, __VA_ARGS__ ) 
...
/**
 * Applies the function macro `f` to each of the remaining parameters and
 * inserts | between the results.
 */
#define MAP_LIST_OR(f, ...) EVAL(MAP_LIST_OR2(f, __VA_ARGS__, ()()(), ()()(), ()()(), 0))
 ...
 * e.g. MAP_LIST_UD(f, x, a, b, c) evaluates to f(a, x), f(b, x), f(c, x)
 */
#define MAP_LIST_UD_OR(f, userdata, ...) EVAL(MAP_LIST2_UD_OR(f, userdata, __VA_ARGS__, ()()(), ()()(), ()()(), 0))
