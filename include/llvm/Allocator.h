...
template <typename AllocatorT = Mallocator, size_t SlabSize = 4096,
...
public:
  ...
  AllocatorStats getAllocatorStats() const {
    ...
    return {
      ...
      .allocated = string_format("Number of memory regions: %zu. Bytes allocated: %zu", Slabs.size(), BytesAllocated),
      .used = string_format("Bytes used: %zu. Bytes wasted: %zu", totalMemory, totalMemory - BytesAllocated)
    };
  }
