...
#ifndef SMALLVECTOR_WITH_ALLOCATOR_INCLUDED
...
protected:
  ...
  void *replaceAllocation(void *NewElts, size_t TSize, size_t OldCapacity, size_t NewCapacity,
                          size_t VSize = 0);
...
template <class T, typename AllocatorT, typename = void> struct SmallVectorAlignmentAndSize {
  alignas(SmallVectorBase<SmallVectorSizeType<T>, AllocatorT>) char Base[sizeof(
      SmallVectorBase<SmallVectorSizeType<T>, AllocatorT>)];
...
template <typename T, typename AllocatorT, typename = void>
...
protected:
  ...
  template<typename AllocT>
  SmallVectorTemplateCommon(size_t Size, AllocT&& allocator) : Base(getFirstEl(), Size, std::forward<AllocT&&>(allocator)) {}
...
template <typename T, typename AllocatorT, bool TriviallyCopyable>
void SmallVectorTemplateBase<T, AllocatorT, TriviallyCopyable>::grow(size_t MinSize) {
  size_t NewCapacity;
...
template <typename T, typename AllocatorT, bool TriviallyCopyable>
T *SmallVectorTemplateBase<T, AllocatorT, TriviallyCopyable>::mallocForGrow(
    size_t MinSize, size_t &NewCapacity) {
  return static_cast<T *>(
      SmallVectorBase<SmallVectorSizeType<T>, AllocatorT>::mallocForGrow(
          this->getFirstEl(), MinSize, sizeof(T), NewCapacity));
}

// Define this out-of-line to dissuade the C++ compiler from inlining it.
template <typename T, typename AllocatorT, bool TriviallyCopyable>
void SmallVectorTemplateBase<T, AllocatorT, TriviallyCopyable>::moveElementsForGrow(
    T *NewElts) {
...
// Define this out-of-line to dissuade the C++ compiler from inlining it.
template <typename T, typename AllocatorT, bool TriviallyCopyable>
void SmallVectorTemplateBase<T, AllocatorT, TriviallyCopyable>::takeAllocationForGrow(
...
class SmallVectorTemplateBase<T, AllocatorT, true> : public SmallVectorTemplateCommon<T, AllocatorT> {
  friend class SmallVectorTemplateCommon<T, AllocatorT>;
...
template <typename T, typename AllocatorT>
class SmallVectorImpl : public SmallVectorTemplateBase<T, AllocatorT> {
...
protected:
  using SmallVectorTemplateBase<T, AllocatorT>::TakesParamByValue;
  using ValueParamT = typename SuperClass::ValueParamT;

  // Default ctor - Initialize to empty.
  explicit SmallVectorImpl(unsigned N)
      : SmallVectorTemplateBase<T, AllocatorT>(N) {}
...
public:
  ...
  // Resizes to size() + N without initializing or constructing any of the new elements
...
template <typename T, typename AllocatorT>
void SmallVectorImpl<T, AllocatorT>::swap(SmallVectorImpl<T, AllocatorT> &RHS) {
  if (this == &RHS) return;
...
template <typename T, typename AllocatorT>
SmallVectorImpl<T, AllocatorT> &SmallVectorImpl<T, AllocatorT>::
  operator=(const SmallVectorImpl<T, AllocatorT> &RHS) {
...
template <typename T, typename AllocatorT>
SmallVectorImpl<T, AllocatorT> &SmallVectorImpl<T, AllocatorT>::operator=(SmallVectorImpl<T, AllocatorT> &&RHS) {
  // Avoid self-assignment.
  if (this == &RHS) return *this;
...
public:
  ...
  template<typename AllocT>
  SmallVector(AllocT&& allocator)
  : SmallVectorImpl<T, AllocatorT>(N, std::forward<AllocT&&>(allocator)) {}
  ...
  explicit SmallVector(size_t Size, const T &Value = T())
    : SmallVectorImpl<T, AllocatorT>(N) {
    this->assign(Size, Value);
  }

  template <typename ItTy, typename = EnableIfConvertibleToInputIterator<ItTy>>
  SmallVector(ItTy S, ItTy E) : SmallVectorImpl<T, AllocatorT>(N) {
    this->append(S, E);
  }
  ...
  SmallVector(std::initializer_list<T> IL) : SmallVectorImpl<T, AllocatorT>(N) {
    this->append(IL);
  }

  template <typename U,
            typename = std::enable_if_t<std::is_convertible<U, T>::value>>
  explicit SmallVector(ArrayRef<U> A) : SmallVectorImpl<T, AllocatorT>(N) {
    this->append(A.begin(), A.end());
  }

  SmallVector(const SmallVector &RHS) : SmallVectorImpl<T, AllocatorT>(N) {
    if (!RHS.empty())
      SmallVectorImpl<T, AllocatorT>::operator=(RHS);
  }
  ...
  SmallVector &operator=(SmallVector &&RHS) {
    ...
    // SmallVectorImpl<T, AllocatorT>::operator= does not leverage N==0. Optimize the
    // case.
    if (this == &RHS)
      return *this;
    if (RHS.empty()) {
  ...
  SmallVector &operator=(SmallVectorImpl<T, AllocatorT> &&RHS) {
    SmallVectorImpl<T, AllocatorT>::operator=(::std::move(RHS));
    return *this;
  }
...
inline size_t capacity_in_bytes(const SmallVector<T, AllocatorT, N> &X) {
...
template <unsigned Size, typename AllocatorT, typename R>
SmallVector<ValueTypeFromRangeType<R>, AllocatorT, Size> to_vector(R &&Range) {
  return {std::begin(Range), std::end(Range)};
}
template <typename R, typename AllocatorT>
SmallVector<ValueTypeFromRangeType<R>, AllocatorT> to_vector(R &&Range) {
  return {std::begin(Range), std::end(Range)};
}

template <typename Out, unsigned Size, typename R, typename AllocatorT>
SmallVector<Out, AllocatorT, Size> to_vector_of(R &&Range) {
  return {std::begin(Range), std::end(Range)};
}

template <typename Out, typename R, typename AllocatorT> SmallVector<Out, AllocatorT> to_vector_of(R &&Range) {
  return {std::begin(Range), std::end(Range)};
}

} // end namespace with alloctor
...
namespace std {
  ...
  template<typename T, typename AllocatorT>
  inline void
  swap(llvm::WithAllocator::SmallVectorImpl<T, AllocatorT> &LHS, llvm::WithAllocator::SmallVectorImpl<T, AllocatorT> &RHS) {
    LHS.swap(RHS);
  }
  ...
  template<typename T, typename AllocatorT, unsigned N>
  inline void
  swap(llvm::WithAllocator::SmallVector<T, AllocatorT, N> &LHS, llvm::WithAllocator::SmallVector<T, AllocatorT, N> &RHS) {
    LHS.swap(RHS);
  }
