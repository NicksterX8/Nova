...
StaticValManager staticManager;
...
struct ComponentWithPtr {
    ...
    void init() {
        model = &myModel;
        staticManager.add(model, MY_MODEL);
    }

    void serialize(/* Outputter out */) {
        
        Uint32 numId = staticManager.registry.at(model);
        const char* name = staticManager.names[numId];
    ...
    static void deserialize(ComponentWithPtr* component /* Inputter in */) {
        Uint32 id;
        // in(&id)
        ...
        int* model = static_cast<int*>(staticManager.get(id));
